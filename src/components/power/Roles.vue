            ...
            <el-row :class="['bd-bottom', index1 ? '' : 'bd-top', 'row-center']" v-for="(item, index1) in scope.row.children" :key="item.id">
              ...
              <el-col :span="19">
                <el-row :class="[index2 ? 'bd-top' : '', 'row-center']" v-for="(item, index2) in item.children" :key="item.id">
                  <!-- 二级菜单 -->
                  <el-col :span="6">
                    <el-tag size="mini" type="success">{{item.authName}}</el-tag>
                    <i class="el-icon-caret-right"></i>
                  </el-col>
                  <!-- 三级菜单 -->
                  <el-col :span="18">
...
export default {
  ...
  methods: {
    ...
    async deleteRightById(role, rightId) {
      // 确认是否删除
      const confirmRes = await this.$confirm('是否永久删除该用户,该操作不可逆是否继续', '删除', {
    ...
    async editRole() {
      ...
      if (!roleDesc || !roleName) return this.$message.error('输入角色以及角色描述有误');
      // 发送编辑角色请求
      const res = await reqSubRoleInfo(this.editRoleForm.roleId, { roleName, roleDesc });
    ...
    async deleteRole(role) {
      const confirmData = await this.$confirm(`是否删除角色'${role.roleName}'`, '删除', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).catch((err) => err);
