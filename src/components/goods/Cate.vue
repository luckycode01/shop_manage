<template>
  <div>
    ...
    <el-card>
      ...
      <!-- 分页区-->
      <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="queryInfo.pagenum" :page-sizes="[3, 5, 7]" :page-size="queryInfo.pagesize" layout="total, sizes, prev, pager, next, jumper" :total="total">
      </el-pagination>
    </el-card>
    <!-- 添加分类对话框 -->
...
export default {
  ...
  methods: {
    async getCateList() {
      const res = await reqGetCateList(this.queryInfo)
    ...
    async editCateDialog(cate) {
      ...
      const res = await reqIdSearchCate(cate.cat_id);
    ...
    editCate() {
      ...
      this.$refs.editCateFormRef.validate(async (valid) => {
        ...
        const res = await reqIdEditCate(cat_id, { cat_name });
    ...
    async deleteCate(cate) {
      ...
      // 发送删除请求
      const res = await reqDeleteCate(cate.cat_id);
